...
namespace BouncyOCSP
{
	class BasicResponseGenerator
    {
		...
		{
			_builder.AddResponse (cert_id, CertificateStatus.Good, DateTime.Now.AddMinutes (5), null);
		}
		public void AddUnknownResponse(CertificateID cert_id)
		{
			_builder.AddResponse (cert_id, new UnknownStatus (), DateTime.Now.AddMinutes (5), null);
		}
		public void AddRevokedResponse(CertificateID cert_id, X509CrlEntry crl_entry)
		{
			...
			_builder.AddResponse (cert_id, status, DateTime.Now.AddMinutes (5), null);
		}
		...
		{
			var status = new RevokedStatus (DateTime.Now, (int)CrlReason.caCompromise);
		...
		public BasicOcspResp Generate()
		{
			...
			var ocsp_resp = _builder.Generate (_algorithm, _token.GetPrivateKey (), new[]{_token.GetOcspSigningCert ()}, DateTime.Now.AddMinutes (5));
			return ocsp_resp;
		}
